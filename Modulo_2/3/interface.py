...
class Interface(tk.Tk):
    def __init__(self):
        ...
        tk.Label(
            sidebar, text="Bandeja de Correo", font=("Arial", 20), bg="#e0e0e0"
        ).pack(pady=18)
        ...
        self.spam_button = tk.Button(
            sidebar, text="Bandeja de Spam", font=("Arial", 16), command=self.show_spam
        )
        self.spam_button.pack(pady=10, fill="x")

        self.new_mail_button = tk.Button(
            sidebar, text="Nuevo Correo", font=("Arial", 16), command=self.open_new_mail
        )
        ...
        self.mail_listbox = tk.Listbox(
            self.main_frame, font=("Arial", 14), selectmode=SINGLE
        )
        self.mail_listbox.pack(side="left", fill="both", expand=True)

        scrollbar = tk.Scrollbar(
            self.main_frame, orient="vertical", command=self.mail_listbox.yview
        )
    ...
    def show_inbox(self):
        ...
        for mail in inbox_mails:
            self.mail_listbox.insert(
                END, mail[:100] + "..." if len(mail) > 100 else mail
            )
    ...
    def show_spam(self):
        ...
        for mail in spam_mails:
            self.mail_listbox.insert(
                END, mail[:100] + "..." if len(mail) > 50 else mail
            )
    ...
    def open_new_mail(self):
        ...
        tk.Label(
            frame, text="Remitente:", font=("Arial", 16), bg="#f5eef5", anchor="w"
        ...
        tk.Label(
            frame, text="Asunto:", font=("Arial", 16), bg="#f5eef5", anchor="w"
